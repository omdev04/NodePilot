...
export default function EditorPage() {
  ...
  const saveFile = async () => {
    ...
    try {
      ...
      // Auto-restart project to apply changes
      setSaveMessage('Saved, restarting project...');
      try {
        // Check if project is running or stopped
        const isRunning = project?.pm2Info?.status === 'online';
        
        if (isRunning) {
          // If running, restart it
          await api.post(`/project/${projectId}/restart`);
        } else {
          // If stopped, start it
          await api.post(`/project/${projectId}/start`);
        }
        
        // Refresh project info to show updated status
        await fetchProject();
        
        setSaveMessage('✓ Saved & restarted successfully');
      } catch (restartError) {
        console.error('Failed to restart project:', restartError);
        setSaveMessage('⚠ Saved but restart failed');
      }
      ...
      return (
        <div key={node.path}>
          ...
          >
            ...
            <span className={`text-sm font-mono ${selectedFile === node.path ? 'text-[#00FF88] font-semibold' : 'text-gray-800 dark:text-gray-300'}`}>
              {node.name}
            </span>
            {node.type === 'file' && (
              <button
                ...
                <div className="h-12 bg-gray-50 dark:bg-white/5 border-b border-gray-200 dark:border-white/10 flex items-center justify-between px-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="p-2 hover:bg-gray-200 dark:hover:bg-gray-800 rounded transition-colors disabled:opacity-30"
                      title="Undo"
                    >
